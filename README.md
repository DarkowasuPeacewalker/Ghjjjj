...
6. В `GameManager` назначь `player`, `spawnPoint`, список сцен (`Level1`, `Level2` при необходимости).
   - Рекомендуется пометить `spawnPoint` тегом `Respawn` (если ссылка не задана вручную).
   - Для авто-поиска UI можно назвать TMP объекты `HPLabel`, `GemsLabel`, `MessageLabel`.
...
Если Unity установлен в твоём окружении, можно собрать так:
